...
const MSubirEclea = ({
  ...
  const generarPDF = () => {
    ...
    doc.text(
      ...
      `${
        dataSelected.person.gender_id === 1
          ? "Masculino"
          : dataSelected.person.gender_id === 2
          ? "Femenino"
          : ""
      }`
    );
    doc.text(100, 147, `${dataSelected.id ? dataSelected.id : ""}`);
    doc.text(
      68,
      168,
    ...
    doc.text(375, 187, getAge());

    doc.text(205, 285, `${result.result_igm ? result.result_igm : ""}`);
    doc.text(295, 285, `${result.result_igg ? result.result_igg : ""}`);
